"""Test Binance USDT-M Futures: connection, data, order lifecycle."""
import sys, os
...
def main():
    report = TestReport("BINANCE_FUTURES")
    try:
        api_key = get_env("BINANCE_LINEAR_API_KEY")
        api_secret = get_env("BINANCE_LINEAR_API_SECRET")
    except EnvironmentError as e:
        report.phase_fail("credentials", str(e)); report.save(); return
    report.phase_ok("credentials", "OK")

    config = TradingNodeConfig(
        trader_id="TEST-BN-FUT-001",
        ...
        exec_clients={},  # BINANCE_LINEAR key lacks trading permissions
    ...
    # node.add_exec_client_factory(BINANCE, BinanceLiveExecClientFactory)  # no trading perms
    node.trader.add_strategy(VenueTestStrategy(
        VenueTestConfig(instrument_id=INSTRUMENT, data_phase_secs=10, max_run_secs=30, supports_modify=True),
        report=report,
    ))
    node.build()
